...
//	private	MQService	sqs			=	new MongoMQService(MONGO_URI);
	...
	private class SwallowPHandler extends SimpleChannelUpstreamHandler {
		...
		public void messageReceived(ChannelHandlerContext ctx, MessageEvent e)
			...
			case PRODUCER_GREET:
				String version = ((PkgProducerGreet) pkg).getProducerVersion();
				System.out.println("Producer version: " + version);
				e.getChannel().write(new PkgSwallowPACK(port));
				break;
			case STRING_MSG:
				PkgStringMessage stringMsg = (PkgStringMessage)pkg;
				System.out.println(stringMsg.getContent());
				e.getChannel().write(new PkgSwallowPACK(stringMsg.getAckNum()));
				System.out.println("ack sent " + port);
				break;
			case BINARY_MSG:
				break;
			default:
