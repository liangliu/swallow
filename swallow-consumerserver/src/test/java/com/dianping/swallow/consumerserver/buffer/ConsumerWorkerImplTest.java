...
public class ConsumerWorkerImplTest extends AbstractDAOImplTest {
   ...
   @Before
   public void before() {
      SwallowBuffer swallowBuffer = Mockito.mock(SwallowBuffer.class);
      when(
            swallowBuffer.createMessageQueue(Mockito.any(String.class), Mockito.any(String.class),
                  Mockito.any(Long.class), Mockito.any(MessageFilter.class))).thenReturn(messageQueue);
      consumerWorkerManager = Mockito.mock(ConsumerWorkerManager.class);
      when(consumerWorkerManager.getSwallowBuffer()).thenReturn(swallowBuffer);
   }
